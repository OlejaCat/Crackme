...
FLAGS = -fstack-protector-strong -fcheck-new -fstrict-overflow
SFML_FLAGS = -lsfml-graphics -lsfml-window -lsfml-system -lsfml-audio 

ifeq ($(DEBUG), 1)
CFLAGS += -Og -g -DDEBUG $(ASAN_FLAGS)
else ifeq ($(RELEASE), 1)
CFLAGS += -O2 -DNDEBUG -g3 $(MARCH) $(FLAGS) -flto
else
CFLAGS += -Og
endif
...
ifeq ($(CC), g++)
CFLAGS += -std=c++17 -Waggressive-loop-optimizations -Wmissing-declarations -Wcast-align -Wcast-qual -Wchar-subscripts -Wconversion -Wempty-body -Wfloat-equal -Wformat-nonliteral -Wformat-security -Wformat-signedness -Wformat=2 -Winline -Wlogical-op -Wopenmp-simd -Wpacked -Wpointer-arith -Winit-self -Wredundant-decls -Wshadow -Wsign-conversion -Wstrict-overflow=2 -Wsuggest-attribute=noreturn -Wsuggest-final-methods -Wsuggest-final-types -Wswitch-default -Wsync-nand -Wundef -Wunreachable-code -Wunused -Wvariadic-macros -Wno-missing-field-initializers -Wno-narrowing -Wno-varargs -Wstack-usage=8192 -Wstack-protector
endif

INCLUDES := -Iinclude
SRCS := $(wildcard source/*.cpp)
OBJS := $(patsubst source/%.cpp, $(BUILD_DIR)/%.o, $(SRCS))
CFLAGS += $(INCLUDES)
...
$(TARGET): $(OBJS)
	@$(CC) $(CFLAGS) -o $(BUILD_DIR)/$(TARGET) $(OBJS) $(SFML_FLAGS)
